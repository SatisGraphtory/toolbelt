...
export class ULocalizationResource {
  // Needs to be a vanilla map of <string, <string, string>> otherwise JSON.stringify won't work :(
  dictionaryEntries = new Map<string, any>();

  constructor(private pakVersion: PakVersion, private reader: Reader,
              public entry: Shape<typeof FPakEntry>, public fileName: string) {
  }
  ...
  async initialize() {
    ...
    for (let i = 0; i < namespaceCount; i++) {
      ...
      for (let k = 0; k < keyCount; k++) {
        ...
        entriesMap.set(key.string, newEntry.localizedString);
      }
      this.dictionaryEntries.set(namespace.string, entriesMap);
    }
  }
